...
class GraderTest(unittest.TestCase):
    ...
    def setUp(self):
        """
        Overrides ASSIGNMENT_GRADER and COURSE_GRADER settings.
        """
        super(GraderTest, self).setUp()
        self.ASSIGNMENT_GRADER = getattr(settings, "ASSIGNMENT_GRADER", None)
        self.COURSE_GRADER = getattr(settings, "COURSE_GRADER", None)
        settings.ASSIGNMENT_GRADER = 'AssignmentFormatGrader'
        settings.COURSE_GRADER = 'WeightedSubsectionsGrader'
        self.addCleanup(self.cleanup)

    def cleanup(self):
        """
        Returns ASSIGNMENT_GRADER and COURSE_GRADER settings to a default value.
        """
        if self.ASSIGNMENT_GRADER:
            settings.ASSIGNMENT_GRADER = self.ASSIGNMENT_GRADER
            self.ASSIGNMENT_GRADER = None
        else:
            del settings.ASSIGNMENT_GRADER

        if self.COURSE_GRADER:
            settings.COURSE_GRADER = self.COURSE_GRADER
            self.COURSE_GRADER = None
        else:
            del settings.COURSE_GRADER
