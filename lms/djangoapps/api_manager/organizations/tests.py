...
class OrganizationsApiTests(TestCase):
    ...
    def test_organizations_metrics_get(self):
        ...
        for i in xrange(1, 6):
            ...
            course1 = CourseFactory.create(display_name="TOMG1", org="CRS1", run="RUN1")
            response = self.do_post(self.base_users_uri, data)
            self.assertEqual(response.status_code, 201)
            ...
            if i < 2:
                StudentGradebook.objects.create(user=user, course_id=course1.id, grade=0.75, proforma_grade=0.85)
            elif i < 4:
                StudentGradebook.objects.create(user=user, course_id=course1.id, grade=0.82, proforma_grade=0.82)
            else:
                StudentGradebook.objects.create(user=user, course_id=course1.id, grade=0.90, proforma_grade=0.91)
