...
class UsersApiTests(TestCase):
    ...
    def test_user_preferences_list_get_default(self):
        ...
        # 'pref-lang' = 'en'
        # Plus a second preference setting when email digests are enabled
    ...
    def test_user_courses_grades_list_get(self):
        ...
        course = CourseFactory.create(org='TUCGLG', run='TUCGLG1', number='101', display_name='TEST USER COURSES GRADES LIST GET')
        test_data = '<html>{}</html>'.format(str(uuid.uuid4()))
        chapter1 = ItemFactory.create(
            category="chapter",
            parent_location=course.location,
        ...
        sequence1 = ItemFactory.create(
            category="sequential",
            parent_location=chapter1.location,
            data=test_data,
            display_name="Sequence 1",
        )
        sequence2 = ItemFactory.create(
            category="sequential",
        ...
        self.assertEqual(courseware_summary[0]['course'], course.display_name)
        self.assertEqual(courseware_summary[0]['display_name'], chapter1.display_name)

        sections = courseware_summary[0]['sections']
        self.assertEqual(len(sections), 1)
        self.assertEqual(sections[0]['display_name'], sequence1.display_name)
        self.assertEqual(sections[0]['graded'], False)

        sections = courseware_summary[1]['sections']
        self.assertEqual(len(sections), 12)
        self.assertEqual(sections[0]['display_name'], sequence2.display_name)
        self.assertEqual(sections[0]['graded'], False)
